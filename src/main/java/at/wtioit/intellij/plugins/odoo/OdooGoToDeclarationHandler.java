...
import at.wtioit.intellij.plugins.odoo.index.OdooJsModuleIE;
import at.wtioit.intellij.plugins.odoo.index.OdooPoMsgIdFileIndex;
import at.wtioit.intellij.plugins.odoo.index.OdooPoMsgIdIE;
...
public class OdooGoToDeclarationHandler extends GotoDeclarationHandlerBase {
    ...
    public @Nullable PsiElement getGotoDeclarationTarget(@Nullable PsiElement psiElement, Editor editor) {
        if (OdooModelPsiElementMatcherUtil.isOdooModelPsiElement(psiElement)) {
        ...
        } else if (OdooRecordPsiElementMatcherUtil.isOdooMenuPsiElement(psiElement)) {
            if (psiElement instanceof XmlToken) {
                return getOdooRecord((XmlToken) psiElement);
            }
        } else if (OdooRecordPsiElementMatcherUtil.isOdooReportPsiElement(psiElement)) {
            if (psiElement instanceof XmlToken) {
                return getOdooRecord((XmlToken) psiElement);
            }
        } else if (OdooRecordPsiElementMatcherUtil.isOdooGroupPsiElement(psiElement)) {
            if (psiElement instanceof XmlToken) {
                return getOdooRecord((XmlToken) psiElement);
            }
        } else if (OdooRecordPsiElementMatcherUtil.isOdooCsvPsiElement(psiElement)) {
            // Переход по XML ID в CSV (id, res_id)
            PsiFile file = psiElement.getContainingFile();
            String text = psiElement.getText();
            // Пробуем найти запись с таким XML ID
            OdooRecordService recordService = file.getProject().getService(OdooRecordService.class);
            OdooRecord record = recordService.getRecord(text);
            if (record != null) {
                return WithinProject.call(file.getProject(), record::getDefiningElement);
            }
            return null;
        } else if (OdooRecordPsiElementMatcherUtil.isOdooJsPsiElement(psiElement)) {
            // Navigate to the JS module definition
            String moduleName = psiElement.getText().replace("'", "").replace("\"", "");
            Project project = psiElement.getProject();
            List<OdooJsModuleIE> result = FileBasedIndex.getInstance().getValues(OdooJsModuleFileIndex.NAME, moduleName, GlobalSearchScope.allScope(project));
            if (!result.isEmpty()) {
            ...
            return null;
        } else if (OdooRecordPsiElementMatcherUtil.isOdooPoPsiElement(psiElement)) {
            // Navigate to the PO/POT msgid definition
            String msgid = psiElement.getText().replace("'", "").replace("\"", "");
            Project project = psiElement.getProject();
            List<OdooPoMsgIdIE> result = FileBasedIndex.getInstance().getValues(OdooPoMsgIdFileIndex.NAME, msgid, GlobalSearchScope.allScope(project));
            if (!result.isEmpty()) {
                String filePath = result.get(0).getFilePath();
                VirtualFile vf = LocalFileSystem.getInstance().findFileByIoFile(new File(filePath));
                if (vf != null) {
