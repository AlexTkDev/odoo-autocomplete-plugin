...
abstract class AbstractOdooCompletionContributor extends CompletionContributor {
    ...
    void suggestJsModuleName(@NotNull CompletionParameters parameters, @NotNull CompletionResultSet result, String value) {
        Project project = parameters.getOriginalFile().getProject();
        PrefixMatcher matcher = result.getPrefixMatcher();

        Collection<String> allModuleNames = com.intellij.util.indexing.FileBasedIndex.getInstance().getAllKeys(at.wtioit.intellij.plugins.odoo.index.OdooJsModuleFileIndex.NAME, project);

        for (String moduleName : allModuleNames) {
            if (result.isStopped()) {
                return;
            }
            if (matcher.prefixMatches(moduleName)) {
                result.addElement(LookupElementBuilder.create(moduleName).withIcon(at.wtioit.intellij.plugins.odoo.icons.OdooPluginIcons.ODOO_TREE_ICON));
            }
        }
    }
    ...
    void suggestPoMsgId(@NotNull CompletionParameters parameters, @NotNull CompletionResultSet result, String value) {
        ...
        Collection<String> allMsgIds = com.intellij.util.indexing.FileBasedIndex.getInstance().getAllKeys(at.wtioit.intellij.plugins.odoo.index.OdooPoMsgIdFileIndex.NAME, project);

        for (String msgid : allMsgIds) {
            if (result.isStopped()) {
                return;
            }
            if (matcher.prefixMatches(msgid)) {
                result.addElement(LookupElementBuilder.create(msgid).withIcon(at.wtioit.intellij.plugins.odoo.icons.OdooPluginIcons.ODOO_TREE_ICON));
            }
        }
    }
}
