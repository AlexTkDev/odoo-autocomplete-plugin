...
public interface OdooModelPsiElementMatcherUtil {
    ...
    static boolean isOdooModelDefinition(@Nullable PsiElement pyline) {
        if (pyline instanceof PyClass) {
            ...
            // NEW: Check for @model or @odoo.model decorator
            if (hasModelDecorator(pyClass)) {
                return true;
            }
        }
        return false;
    ...
    // NEW: Helper to check for @model or @odoo.model decorator
    static boolean hasModelDecorator(PyClass pyClass) {
        if (pyClass == null) return false;
        PsiElement firstChild = pyClass.getFirstChild();
        if (firstChild instanceof PyDecoratorList) {
